...
// InherentRiskDao is needed because wire does not directly support
// generics, but does appear to support injecting non-generic types
// that contain embedded types
type InherentRiskDao interface {
	...
	//  3. run `go install ./...`
	servicedao.TypedDao[m.InherentRisk]
}

func NewInherentRiskCloverDao(db clover.CloverDbWrapper, config InherentRiskCloverCollectionConfig) *clover.CloverDao[InherentRiskCloverCollectionConfig] {
