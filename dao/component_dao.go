...
// ComponentDao is needed because wire does not directly support
...
type ComponentDao interface {
	...
	//  3. run `go install ./...`
	servicedao.TypedDao[m.Component]
}

func NewComponentCloverDao(db clover.CloverDbWrapper, config ComponentCloverCollectionConfig) *clover.CloverDao[ComponentCloverCollectionConfig] {
