...
func TestGetThreatModelHandler(t *testing.T) {
	...
	var tests = []struct {
		...
		expectedBodyMap  map[string]string // not checked if nil
	}{
		{
		...
		{
			"should return 404 for non-existent threatModel",
			&m.AuthenticationInfo{UserID: "u-12345678", Roles: []m.Role{m.RoleUser}},
			nil,
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			request, _ := http.NewRequest(http.MethodGet,
				server.URL+"/api/v1/threatModel/by-id/"+string(threatModel.ThreatModelID), nil)
			...
			if test.expectedBodyMap != nil {
				got := map[string]string{}
				body := readToBytes(response.Body)
				err = json.Unmarshal(body, &got)
				require.Nil(t, err)

				require.Equal(t, got, test.expectedBodyMap)
			}
		})
	}
}
...
func TestCreateThreatModelHandler(t *testing.T) {
	...
	var tests = []struct {
		...
		//
		{
			...
			msi{"groupID": "g-1234-1234", "name": "my new threatModel", "userID": "u-1234-1234"},
			nil,
			nil,
			nil,
			nil,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.ai != nil {
				...
				s, err := structs.StructToJSON(test.inputThreatModel)
				require.Nil(t, err)
			...
			request, _ := http.NewRequest(http.MethodPatch,
				server.URL+"/api/v1/threatModel/by-id/"+string(test.inputThreatModelID), bodyReader)
			response, err := http.DefaultClient.Do(request)
